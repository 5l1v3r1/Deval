package r1.deval.rt
{
  public class ClassDef
  {
	...
	public function ClassDef(classname:String, classes:Object, staticExprs:Object, varExprs:Object, functionExprs:Object, importStmts:Array, staticgetters:Object, staticsetters:Object, vargetters:Object, varsetters:Object):void
	{
	  ...
	  for (var v:String in this.staticExpressions) this.classObj[v] = undefined;
	  ...
	  for (v in this.staticgetters) this.classObj.deval_namesp::addGetter(v, null);
	  for (v in this.staticsetters) this.classObj.deval_namesp::addSetter(v, null);
	...
	private function doInit():void
	{
	  ...
	  try
	  {
		...
		for (s in this.staticsetters) this.classObj.deval_namesp::addSetter(s, this.staticsetters[s].getFunction());
		this.staticsetters = null;
		for each(var m:ImportStmt in this.importStmts) m.exec();
		this.importStmts = null;
		for (s in this.staticExpressions)
		{
		  this.classObj[s] = v[s];
		  delete v[s];
		}
	...
	public function getInstance(...args):Object
	{
	  ...
	  for (s in this.varsetters) ths.deval_namesp::addSetter(s, this.varsetters[s].getFunction(ths));
	  try
	  {
		for (s in this.functionExprs) ths[s] = this.functionExprs[s].getFunction(ths);
		for (s in this.varExprs) m.deval_namesp::addProperty(s, this.varExprs[s], false);
		for (s in this.varExprs)
		{
		  ths[s] = m[s];
		  delete m[s];
		}
